...
/*   Updated: 2015/06/23 16:31:53 by mbarbari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(std::string name, int grade) : _name(name), _grade(grade), _gradeSign(false){
	if (grade > 150)
...
Form::Form(std::string name) : _name(name), _grade(1){
}

Form::Form(Form const &rhs) : _name(rhs.getName()), _grade(rhs.getGrade()), _gradeSign(rhs._gradeSign){
	if (rhs.getGrade() > 150)
		throw Form::GradeTooLowException();
...
const char	*Form::GradeTooLowException::what() const throw() {
	...
	else
	{
		...
		ofs << this->_gradeVal;;
		return (std::string(this->_name + " are too low for signed this formulaire\n").c_str());
	}
}
